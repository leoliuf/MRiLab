/* Chemical Exchange Saturation Transfer (CEST) Bloch Equation Kernel running at CUDA GPU */

#ifndef _BLOCH_KERNEL_CEST_GPU_H_
#define _BLOCH_KERNEL_CEST_GPU_H_

__global__ void
BlochKernelCESTGPU(float Gyro, double *d_CS, float *d_Rho, float *d_T1, float *d_T2, float *d_K, float *d_Mz, float *d_My, float *d_Mx, float *d_Buffer,
				 ...
				 int SpinMxX, int SpinMxY, int SpinMxZ, int SpinNum, int TypeNum, int TxCoilNum, int RxCoilNum, int SeqLen)
{
	...
	for (int s=0; s < SpinNum; s++){
		for (int k=0; k < SpinMxZ; k++){
			...
			/* MT exchange term */
			p_d_K12			= d_K 			+ k * SpinMxNum	+ tid 	+ 0 * SpinMxAllNum;
			p_d_K21			= d_K 			+ k * SpinMxNum	+ tid 	+ (TypeNum-1) * SpinMxAllNum;
			K12 					= *p_d_K12;
			K21 					= *p_d_K21;
			...
			for (int q=0; q< SeqLen; q++){
				...
				if (RunMode == 1){
					/* free pool */
					p_d_b_Mz  = d_b_Mz 	+ (utsi - SeqLen + q) * SpinMxAllNum * SpinNum * TypeNum	+ k * SpinMxNum	+ tid  + 0 * (SpinMxAllNum * SpinNum)  + s * SpinMxAllNum;
					p_d_b_My  = d_b_My 	+ (utsi - SeqLen + q) * SpinMxAllNum * SpinNum * TypeNum	+ k * SpinMxNum	+ tid  + 0 * (SpinMxAllNum * SpinNum)  + s * SpinMxAllNum;
					p_d_b_Mx  = d_b_Mx 	+ (utsi - SeqLen + q) * SpinMxAllNum * SpinNum * TypeNum	+ k * SpinMxNum	+ tid  + 0 * (SpinMxAllNum * SpinNum)  + s * SpinMxAllNum;
					*p_d_b_Mz = Mz;
					*p_d_b_My = My;
					*p_d_b_Mx = Mx;
					
					/* bound pool */
					p_d_b_Mz  = d_b_Mz 	+ (utsi - SeqLen + q) * SpinMxAllNum * SpinNum * TypeNum	+ k * SpinMxNum	+ tid  + 1 * (SpinMxAllNum * SpinNum)  + s * SpinMxAllNum;
					*p_d_b_Mz = Mzb;

					/* CEST pool */
					for (int t=2; t < TypeNum; t++){ 
				...
				if (*p_d_ADC == 1) {
					for (int c = 0; c < RxCoilNum; c++){  /* signal acquisition per Rx coil */    
						...
						/* signal buffer pointer */ /* ? is it right for only collecting signal for free pool ? */
				...
				/* buffer Mx, My, Mz for CEST exchange */
				for (int t=2; t < TypeNum; t++){
					p_d_Mx       = d_Mx 			+ k * SpinMxNum	+ tid 	+ t * (SpinMxAllNum * SpinNum) 	+ s * SpinMxAllNum;
					p_d_My       = d_My 			+ k * SpinMxNum	+ tid 	+ t * (SpinMxAllNum * SpinNum) 	+ s * SpinMxAllNum;
					p_d_Mz       = d_Mz 			+ k * SpinMxNum	+ tid 	+ t * (SpinMxAllNum * SpinNum) 	+ s * SpinMxAllNum;
				...
				p_d_Buffer[(0 * TypeNum + 0) * SpinMxNum]    = Mx;
				p_d_Buffer[(1 * TypeNum + 0) * SpinMxNum]    = My;
				p_d_Buffer[(2 * TypeNum + 0) * SpinMxNum]    = Mz;
				...
				/* CEST exchange for free pool */
				ExpdtKout = 1;
				ExpdtKxin  = 0;
				ExpdtKyin  = 0;
				ExpdtKzin  = 0;
				for (int t=2; t< TypeNum; t++){
					/* go away to CEST pool */
					p_d_K	  = d_K   +  k * SpinMxNum + tid   + (t - 1) * SpinMxAllNum; 
					if (*p_d_K != 0 ){
						ExpdtKout *= exp(-(*p_d_dt) * (*p_d_K));
					}
												
					/* come in from CEST pool */
					p_d_K	  = d_K   +  k * SpinMxNum + tid   + (t + TypeNum - 2) * SpinMxAllNum; 
					if (*p_d_K != 0 ){
						ExpdtKxin += (1-exp(-(*p_d_dt) * (*p_d_K))) * p_d_Buffer[(0 * TypeNum + t) * SpinMxNum];
						ExpdtKyin += (1-exp(-(*p_d_dt) * (*p_d_K))) * p_d_Buffer[(1 * TypeNum + t) * SpinMxNum];
						ExpdtKzin += (1-exp(-(*p_d_dt) * (*p_d_K))) * p_d_Buffer[(2 * TypeNum + t) * SpinMxNum];
					}
				}
				
				Mx = bufferMx * ExpdtT2 * ExpdtKout + ExpdtKxin;
				My = bufferMy * ExpdtT2 * ExpdtKout + ExpdtKyin;
				Mz = bufferMz * ExpdtT1 * ExpdtKout + ExpdtKzin + Mzbbuffer * ExpdtK + M0dtT1;
				
				/*********************************** CEST pool spin evolution *******************************/
				for (int t=2; t < TypeNum; t++){
					p_d_Mx       = d_Mx 			+ k * SpinMxNum	+ tid 	+ t * (SpinMxAllNum * SpinNum) 	+ s * SpinMxAllNum;
					p_d_My       = d_My 			+ k * SpinMxNum	+ tid 	+ t * (SpinMxAllNum * SpinNum) 	+ s * SpinMxAllNum;
					p_d_Mz       = d_Mz 			+ k * SpinMxNum	+ tid 	+ t * (SpinMxAllNum * SpinNum) 	+ s * SpinMxAllNum;
					...
					if (rfAmpSum != 0){
						...
						bufferMx = Mxc * (cosPhi*buffer1 + sinPhi*(cosAlpha*sinPhi + cosPhi*sinAlpha*sinBeta))
										-Myc * (sinPhi*buffer1 - cosPhi*(cosAlpha*sinPhi + cosPhi*sinAlpha*sinBeta))
									   +Mzc * (cosBeta*(sinAlpha*sinPhi - cosAlpha*cosPhi*sinBeta) + cosBeta*cosPhi*sinBeta);  /*Calculate Mx */
						
						bufferMy = Myc * (sinPhi*buffer2 + cosPhi*(cosAlpha*cosPhi - sinAlpha*sinBeta*sinPhi))
										-Mxc * (cosPhi*buffer2 - sinPhi*(cosAlpha*cosPhi - sinAlpha*sinBeta*sinPhi))
									   +Mzc * (cosBeta*(cosPhi*sinAlpha + cosAlpha*sinBeta*sinPhi) - cosBeta*sinBeta*sinPhi);  /*Calculate My */
						
						bufferMz = Mxc * (cosPhi*(cosBeta*sinBeta - cosAlpha*cosBeta*sinBeta) - cosBeta*sinAlpha*sinPhi)
										-Myc * (sinPhi*(cosBeta*sinBeta - cosAlpha*cosBeta*sinBeta) + cosBeta*cosPhi*sinAlpha)
									   +Mzc * (cosAlpha*pow(cosBeta,2) + pow(sinBeta,2));     /*Calculate Mz */
					}
					else{
						Alpha	 = dW * (*p_d_dt);  /* calculate alpha */
						sinAlpha = sin(Alpha);
						...
						bufferMx = Mxc * cosAlpha + Myc * sinAlpha;     /* calculate Mx */
						bufferMy = Myc * cosAlpha - Mxc * sinAlpha;     /* calculate My */
						bufferMz = Mzc ;								  /* calculate Mz */
					}
					
					/* relax */
					if (dt != *p_d_dt){ /* exp & division is very time consuming */
						p_d_Buffer[(3 * TypeNum + t) * SpinMxNum] = exp(-(*p_d_dt)/(T2c));
						p_d_Buffer[(4 * TypeNum + t) * SpinMxNum] = exp(-(*p_d_dt)/(T1c));
						p_d_Buffer[(5 * TypeNum + t) * SpinMxNum] = (Rhoc*(1 - p_d_Buffer[(4 * TypeNum + t) * SpinMxNum]))/SpinNum;
					}
					...
					ExpdtKyin  = 0;
					ExpdtKzin  = 0;
					/* go away to free pool */
					p_d_K	  = d_K   +  k * SpinMxNum + tid   + (t + TypeNum - 2) * SpinMxAllNum; 
					if (*p_d_K != 0 ){
						ExpdtKout = exp(-(*p_d_dt) * (*p_d_K));
					}
					
					/* come in from free pool */
					p_d_K	  = d_K   +  k * SpinMxNum + tid   + (t - 1) * SpinMxAllNum; 
					if (*p_d_K != 0 ){
						ExpdtKxin = (1-exp(-(*p_d_dt) * (*p_d_K))) * p_d_Buffer[(0 * TypeNum + 0) * SpinMxNum];
						ExpdtKyin = (1-exp(-(*p_d_dt) * (*p_d_K))) * p_d_Buffer[(1 * TypeNum + 0) * SpinMxNum];
						ExpdtKzin = (1-exp(-(*p_d_dt) * (*p_d_K))) * p_d_Buffer[(2 * TypeNum + 0) * SpinMxNum];
					}
					
					Mxc = bufferMx * ExpdtKout + ExpdtKxin;
					Myc = bufferMy * ExpdtKout + ExpdtKyin;
					Mzc = bufferMz * ExpdtKout + ExpdtKzin + p_d_Buffer[(5 * TypeNum + t) * SpinMxNum]; 
                ...
                if (rfAmpSum != 0){
                    if (TxCoilNum == 1) { /* single-Tx */
                        ...
                        */
                        if (rffreq != rfFreq || t2b != T2b){ /* SuperLorentian lineshape -- this is very time consuming */
							rffreq = rfFreq;
							t2b = T2b;
							u=0;
							G1=0;
							for (int t=0; t < 1000; t++){
								...
								G1+=(sqrt(2/PI)*(T2b/fabs(3*pow(u,2)-1))*exp(-2*pow(buffer2,2)))*0.001;
								u+=0.001;
							}
						}
                ...
                /* relax & exchange*/
				if (dt != *p_d_dt){ /* exp & division is very time consuming */
					ExpdtT1b = exp(-*p_d_dt/T1b - *p_d_dt*K21);
					M0dtT1b	= (Rhob*(1-exp(-*p_d_dt/T1b)))/SpinNum;
					ExpdtKb	 = 1-exp(-*p_d_dt*K12);
				}
